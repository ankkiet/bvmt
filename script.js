...
let aiKeys = [{name: "Mặc định", val: "AIzaSyAnOwbqmpQcOu_ERINF4nSfEL4ZW95fiGc"}]; // Default
...
// --- DETECT WEBVIEW (ZALO/MESSENGER FIX) ---
...
// --- 1. GLOBAL NOTIFICATION SYSTEM (ADMIN POPUP) ---
...
// --- 2. PERSONAL NOTIFICATION SYSTEM (LIKE/COMMENT) ---
let notifUnsub = null;
...
function listenToMyNotifications(uid) {
    ...
    notifUnsub = onSnapshot(q, (snap) => {
        ...
        notifs.forEach(data => {
            ...
            html += `
            <div class="notif-item ${data.isRead ? '' : 'unread'}" onclick="clickNotification('${data.id}', '${data.collectionRef}', '${data.link}')">
...
async function pushNotification(recipientId, type, message, linkId, colRef) {
    ...
    try {
        ...
        console.log("Push notif success");
    } catch (e) {
...
window.clickNotification = async (notifId, col, postId) => {
    ...
    if(col && postId && col !== 'undefined') {
        window.openLightbox(col, postId);
    }
...
async function callGeminiAPI(prompt, imageBase64 = null) {
    ...
    for (let i = 0; i < aiKeys.length; i++) {
        ...
        try {
            ...
            if (i === aiKeys.length - 1) return "Tất cả Key AI đều bận hoặc lỗi.";
        }
    }
}
...
window.fillChat = (text) => {
    ...
    window.sendMessageToAI(new Event('submit'));
}
...
window.sendMessageToAI = async (e) => {
    ...
    msgList.innerHTML += `<div class="ai-msg user">${msg}</div>`; 
    input.value = ""; 
    msgList.scrollTop = msgList.scrollHeight;
    
    const loadingId = "ai-loading-" + Date.now(); 
    msgList.innerHTML += `<div class="ai-msg bot" id="${loadingId}"><i class="fas fa-ellipsis-h fa-fade"></i></div>`; 
    msgList.scrollTop = msgList.scrollHeight;
    
    try { 
        const responseText = await callGeminiAPI(msg); 
        document.getElementById(loadingId).innerText = responseText; 
    } catch(err) { 
...
function onPlayerStateChange(event) { const icon = document.getElementById('music-icon-display'); if(event.data == YT.PlayerState.PLAYING) { icon.classList.add('playing'); icon.style.color = 'var(--primary)'; } else { icon.classList.remove('playing'); icon.style.color = '#333'; } }
...
onSnapshot(doc(db, "settings", "config"), (docSnap) => {
    if(docSnap.exists()) {
        ...
        if(cfg.aiKeys && cfg.aiKeys.length > 0) { 
            ...
            if(list) { 
                list.innerHTML = ""; 
                aiKeys.forEach(k => { list.innerHTML += `<div class="key-item"><span class="key-name">${k.name}</span><span class="key-val">******</span><button class="btn btn-sm btn-danger" onclick="removeAIKey('${k.name}', '${k.val}')">X</button></div>`; }); 
            } 
        }
...
window.executeUpload = async (i) => { 
    ...
    let aiPrompt = isTrash 
        ? "Đây là loại rác gì? Nó thuộc nhóm (Hữu cơ, Tái chế, hay Rác thải còn lại)? Hãy hướng dẫn cách vứt ngắn gọn." 
        : "Đóng vai một học sinh lớp A2K41 đăng ảnh lên mạng xã hội của lớp. Hãy viết 3 dòng trạng thái (caption) ngắn gọn, tự nhiên, xưng hô 'mình' hoặc 'lớp tớ' về bức ảnh này. Gợi ý 1: Vui vẻ. Gợi ý 2: Ý nghĩa. Gợi ý 3: Hài hước. Mỗi gợi ý 1 dòng gạch đầu dòng."; 
    
    let description = ""; 
    if(!isTrash) { 
        const d = prompt("Nhập mô tả cho ảnh (Hoặc để trống để AI gợi ý caption):"); 
        if(d === null) return; 
        description = d; 
    } 
    
    document.getElementById('upload-loading-text').innerText = isTrash ? "AI đang soi rác..." : "AI đang viết caption..."; 
        ...
        if(j.secure_url) { 
            if(isTrash || !description) { 
                try { 
                    ...
                    console.error(err); if(isTrash) alert("AI lỗi, không thể phân loại."); 
                } 
            } 
    ...
    const unsub = onSnapshot(query(collection(db, col), where("archived", "!=", true)), (snap) => {
        ...
        if(col === 'gallery' && docs.length > 0) {
            ...
            if(topPost) {
                ...
                document.getElementById('feat-img').src = topPost.url; document.getElementById('feat-title').innerText = "TOP 1 ĐƯỢC YÊU THÍCH"; document.getElementById('feat-desc').innerText = topPost.desc; document.getElementById('feat-author').innerText = "— " + topPost.authorName;
            }
        }
...
window.openLightbox = async (c, i) => { 
    ...
    const imgArea = document.getElementById('lb-zoom-area'); 
    imgArea.classList.remove('zoomed'); 
    const imgEl = document.getElementById('lb-img');
    imgEl.style.transform = "scale(1)"; 
    
    imgEl.src=d.url; 
    document.getElementById('lb-author-avatar').src=d.authorAvatar||'https://via.placeholder.com/35'; 
    ...
    if(currentUser && d.likes?.includes(currentUser.uid)) { 
        btn.classList.add('liked'); btn.style.color='#e53935'; 
    } else { 
    ...
    if(currentUser && (currentUser.uid === d.uid || isAdmin(currentUser.email))) {
        ...
        document.querySelector('.lb-btn-pin').style.display = isAdmin(currentUser.email) ? 'block' : 'none';
    } else {
...
let lastTap = 0;
const imgEl = document.getElementById('lb-img');
const zoomArea = document.getElementById('lb-zoom-area');
...
window.quickReply = async (text) => {
    ...
    fakeDiv.className = 'lb-comment-item';
    ...
    if(postSnap.exists()){
        const ownerId = postSnap.data().uid;
        pushNotification(ownerId, 'comment', `<b>${currentUser.displayName}</b> đã bình luận: "${text}"`, currentImgId, currentCollection);
    }
}
...
window.handleLike = async () => { 
    ...
    const btn = document.getElementById('lb-like-btn');
    const countSpan = document.getElementById('lb-like-count');
    ...
    if (isLiked) {
        ...
        if(postSnap.exists()){
            const ownerId = postSnap.data().uid;
            pushNotification(ownerId, 'like', `<b>${currentUser.displayName}</b> đã thả tim ảnh của bạn ❤️`, currentImgId, currentCollection);
        }
    }
}
...
function renderComments(arr) { 
    ...
    arr.forEach(c=>{ 
        l.innerHTML+=`<div class="lb-comment-item"><img src="${c.avatar||'https://via.placeholder.com/30'}" class="lb-comment-avatar"><div class="lb-comment-content"><div class="lb-comment-bubble"><span class="lb-comment-user">${c.name}</span><span class="lb-comment-text">${c.text}</span></div></div></div>`; 
    }); 
...
window.exportExcel = async (type) => { 
    ...
    if (type === 'users') { 
        sheet.columns = [ 
            ...
            { header: 'Ngày đăng ký', key: 'created', width: 20 }, 
            { header: 'Lớp', key: 'class', width: 15 }, 
            { header: 'Hoạt động cuối', key: 'active', width: 20 }, 
            { header: 'Số lần đăng nhập', key: 'count', width: 15 } 
        ]; 
        const snap = await getDocs(collection(db, "users")); 
        snap.forEach(d => { 
            const u = d.data(); 
            sheet.addRow({ name: u.displayName || '', email: u.email || '', id: u.customID || '', created: u.createdAt ? new Date(u.createdAt.seconds * 1000).toLocaleString('vi-VN') : '', class: u.class || '', active: u.lastActive ? new Date(u.lastActive.seconds * 1000).toLocaleString('vi-VN') : '', count: u.loginCount || 1 }); 
        }); 
    } else { 
        sheet.columns = [ 
        ...
        snap.forEach(d => { 
            const p = d.data(); 
            sheet.addRow({ author: p.authorName, uid: p.authorID || '', class: p.className || '', desc: p.desc || '', likes: p.likes ? p.likes.length : 0, url: p.url, date: p.createdAt ? new Date(p.createdAt.seconds * 1000).toLocaleString('vi-VN') : '' }); 
        }); 
    } 
...
installLi.innerHTML = '<a id="btn-install-pc" style="display:none; color:yellow; cursor:pointer"><i class="fas fa-download"></i> Tải App</a>';
pcMenu.appendChild(installLi);
const mobNav = document.querySelector('.mobile-nav-inner');
const installMob = document.createElement('a');
installMob.className = 'nav-item';
installMob.id = 'btn-install-mob';
installMob.style.display = 'none';
installMob.innerHTML = '<i class="fas fa-download"></i><span>TảiApp</span>';
mobNav.insertBefore(installMob, mobNav.firstChild);
